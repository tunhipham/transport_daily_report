...
```
┌─────────────────────────────────────────────────────────┐
│ Watch Mode (auto_compose.py --watch)                     │
│                                                          │
│  Poll Drive ─→ Detect KH file ─→ Fetch data ─→ Compose  │
│                                         │                │
│                                     inject_haraworks.py  │
│                                         │                │
│  ┌─────────────────────────────────────────────────┐     │
│  │ 1. Mở Edge (.edge_automail profile)             │     │
│  │ 2. Check login → auto-wait nếu cần             │     │
│  │ 3. Tìm thread W{week} trong Inbox → reply      │     │
│  │    (nếu ko có → compose mail mới)               │     │
│  │ 4. Điền Subject                                 │     │
│  │ 5. Inject HTML vào CKEditor (JS setData)        │     │
│  │ 6. Auto-save draft → đóng browser               │     │
│  │ ⚠ KHÔNG BẤM GỬI                                │     │
│  └─────────────────────────────────────────────────┘     │
│                                                          │
│  User: mở Haraworks → review draft → thêm To/CC → Gửi  │
└─────────────────────────────────────────────────────────┘
```
