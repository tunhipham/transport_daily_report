...
# Original data location (kept for reference/future use)
DATA_DIR = r'G:\My Drive\DOCS\DAILY\LICH DI HANG DC'
...
TELEGRAM_CONFIG = os.path.join(REPO_ROOT, 'config', 'telegram_nso.json')
SENT_MSG_FILE = os.path.join(REPO_ROOT, 'output', 'state', 'nso', 'sent_messages.json')

def load_telegram_config():
    """Load bot_token and chat_id from config/telegram.json."""
...
def save_sent_messages(data):
    ...
    with open(SENT_MSG_FILE, 'w', encoding='utf-8') as f:
        json.dump(data, f, indent=2, ensure_ascii=False)

def delete_telegram_message(message_id, bot_token, chat_id):
    """Delete a message by ID via Telegram Bot API."""
    ...
    try:
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            if result.get('ok'):
                print(f"  🗑️  Telegram  : Xóa tin nhắn cũ #{message_id}")
            else:
                print(f"  ⚠️  Telegram  : Không xóa được #{message_id} — {result.get('description', '')}")
    except Exception as e:
        print(f"  ⚠️  Telegram  : Lỗi xóa #{message_id} — {e}")

def delete_old_nso_messages(bot_token, chat_id):
    """Delete all previously sent NSO messages."""
    sent = load_sent_messages()
    old_ids = sent.get('nso_message_ids', [])
    if old_ids:
        print(f"  🗑️  Xóa {len(old_ids)} tin nhắn NSO cũ...")
        for mid in old_ids:
            delete_telegram_message(mid, bot_token, chat_id)
    # Clear saved IDs
    sent['nso_message_ids'] = []
    save_sent_messages(sent)

def send_telegram_document(caption, file_path, bot_token, chat_id):
    """Send an HTML file as document via Telegram Bot API with caption. Returns message_id."""
    import uuid
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as resp:
            ...
            if result.get('ok'):
                ...
                print(f"  ✅ Telegram  : Gửi file + thông báo thành công! (msg #{msg_id})")
...
def send_telegram_photo(photo_path, bot_token, chat_id):
    """Send a photo via Telegram Bot API. Returns message_id."""
    import uuid
    boundary = uuid.uuid4().hex
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as resp:
            ...
            if result.get('ok'):
                msg_id = result['result']['message_id']
                print(f"  ✅ Telegram  : Gửi ảnh calendar thành công! (msg #{msg_id})")
                sent = load_sent_messages()
                ids = sent.get('nso_message_ids', [])
                ids.append(msg_id)
                sent['nso_message_ids'] = ids
