...
STATE_PATH = os.path.join(BASE, "output", "auto_compose_state.json")
OUTPUT = os.path.join(BASE, "output")

# ── Logging ──────────────────────────────────────────────────────────
LOG_PATH = os.path.join(BASE, "output", "auto_compose.log")
...
def load_plan_data(week, kho, date_str, session=None):
    ...
    """
    json_path = os.path.join(OUTPUT, f"weekly_plan_{week}.json")
...
def fetch_latest_data(week, week_start):
    ...
    cmd = [
        sys.executable,
        os.path.join(BASE, "script", "fetch_weekly_plan.py"),
        "--week", week,
        "--start", week_start,
    ]
...
def compose_for_kho(kho, session, date_str, week):
    ...
    cmd = [
        sys.executable,
        os.path.join(BASE, "script", "compose_mail.py"),
        "--kho", kho,
        "--date", date_str,
        "--week", week,
    ]
...
def inject_to_haraworks(kho, session, date_str, week):
    ...
    cmd = [
        sys.executable,
        os.path.join(BASE, "script", "inject_haraworks.py"),
        "--kho", kho,
        "--date", date_str,
        "--week", week,
    ]
