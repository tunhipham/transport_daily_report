...
def copy_html_to_clipboard(html_content):
    ...
    tmp_path = os.path.join(OUTPUT, "_mail_body.html")
...
"""
    ...
    js_path = os.path.join(OUTPUT, "_mail_inject.js")
...
def main():
    ...
    if not args.week:
        files = [f for f in os.listdir(OUTPUT) if f.startswith("weekly_plan_W") and f.endswith(".json")]
        if files:
            files.sort()
