...
def parse_schedule_days(schedule_str):
    """Parse 'Thứ 3-5-7' or 'Hàng ngày' → set of weekday indices (Mon=0)."""
...
def apply_nso(stores, week_dates):
    ...
    for nso in NSO_STORES:
        ...
        # Existing store data (name, shift, schedule) is LOCKED.
        # NSO only writes châm hàng days — never overwrites existing data.
        # Wrong-code protection is handled upstream (fetch_nso_mail NKT check).
        matched = None
