...
DATA = os.path.join(BASE, "data", "raw", "daily")
            ...
            try:
                ...
                for hdr_row in ws.iter_rows(min_row=1, max_row=1, values_only=False):
                    for i, cell in enumerate(hdr_row):
                        ...
                        if ("kien" in h or "kiến" in h) and "giao" in h:
                            actual_time_col = i  # Best: "du kien giao"
                        elif ("kien" in h or "kiến" in h) and ("roi" not in h and "rời" not in h) and time_col_backup is None:
                            time_col_backup = i  # Backup: "TG DỰ KIẾN" (not "rời kho")
                        elif "loại" in h or "loai" in h:
                            actual_loai_col = i
                # Use backup if primary detection didn't find a match
                if actual_time_col == time_col and time_col_backup is not None:
                    actual_time_col = time_col_backup
