...
NOI_CHUYEN_MAP = {
    "KSL": "DRY",
    "SLKT": "KSL-Tối",  # SLKT = KSL Tối (always night shift)
    "KRC": "KRC",
    "QCABA": "ĐÔNG MÁT",
}
...
def _serialize_row(r):
    """Convert row dict to serializable format for cache (time→str)."""
    out = dict(r)
    if out.get("date"):
        out["date"] = out["date"].isoformat()
    if out.get("arrival_time"):
        out["arrival_time"] = out["arrival_time"].isoformat()
    if out.get("arrival_dt"):
        out["arrival_dt"] = out["arrival_dt"].isoformat()
    return out


def _deserialize_row(r):
    """Restore row dict from cache (str→time/date)."""
    from datetime import date as ddate
    out = dict(r)
    if out.get("date"):
        out["date"] = ddate.fromisoformat(out["date"])
    if out.get("arrival_time"):
        out["arrival_time"] = dtime.fromisoformat(out["arrival_time"])
    if out.get("arrival_dt"):
        out["arrival_dt"] = datetime.fromisoformat(out["arrival_dt"])
    else:
        out["arrival_dt"] = None
    return out
...
def load_trip_data(month, year):
    """Load and dedup trip data — incremental with cache.
    
    Only reads NEW xlsx files not yet in cache. Previously processed files
    are locked (loaded from cache) to avoid re-reading large files.
    Cache: output/trip_cache_T{mm}.json
    """
    import openpyxl
    data_dir = rf'G:\My Drive\DOCS\DAILY\DS chi tiet chuyen xe\T{month:02d}.{year % 100}'
    cache_path = os.path.join(OUTPUT, "state", f"trip_cache_T{month:02d}.json")
    ...
    # Load cache
    cached_files = set()
    cached_rows = {}  # (trip_id, dest, sub_kho) -> row dict
    if os.path.exists(cache_path):
        try:
            with open(cache_path, "r", encoding="utf-8") as f:
                cache_data = json.load(f)
            cached_files = set(cache_data.get("processed_files", []))
            for r in cache_data.get("rows", []):
                row = _deserialize_row(r)
                key = (row["trip_id"], row["dest"], row.get("sub_kho", ""))
                cached_rows[key] = row
        except Exception as e:
            print(f"  ⚠ Cache read error, starting fresh: {e}")
            cached_files = set()
            cached_rows = {}
    
    # Determine which files are new
    new_files = [f for f in files if f not in cached_files]
    locked_files = [f for f in files if f in cached_files]
    
    if locked_files:
        print(f"  🔒 Locked (cached): {len(locked_files)} files ({', '.join(locked_files)})")
        print(f"     → {len(cached_rows)} rows from cache")
    
    if not new_files:
        print(f"  ✅ No new files — using cache only")
        rows = list(cached_rows.values())
        with_time = [r for r in rows if r["arrival_time"] is not None]
        no_time = len(rows) - len(with_time)
        print(f"  📊 {len(rows)} deduped rows from cache")
        print(f"  🕐 {len(with_time)} with arrival time, {no_time} without")
        return rows
    
    print(f"  🆕 New files to process: {len(new_files)} ({', '.join(new_files)})")
    
    # Start from cached data
    seen = dict(cached_rows)
    total_raw = 0
    
    for fname in new_files:
        fpath = os.path.join(data_dir, fname)
        try:
            file_rows, file_raw = _load_single_file(fpath)
            total_raw += file_raw
            # Merge: new data can override cached data (newer file wins)
            for key, row in file_rows.items():
                seen[key] = row
            print(f"    ✅ {fname}: {file_raw} raw → {len(file_rows)} deduped")
        except Exception as e:
            print(f"    ⚠ Error reading {fname}: {e}")
    
    rows = list(seen.values())
    print(f"  📊 {total_raw} new raw rows + {len(cached_rows)} cached → {len(rows)} total deduped")
    
    # Save updated cache
    try:
        all_processed = sorted(cached_files | set(new_files))
        cache_out = {
        ...
        print(f"  💾 Cache saved: {cache_path} ({len(all_processed)} files locked)")
    except Exception as e:
...
def load_thitca_data(months):
    ...
    all_rows = []
    for m in months:
        plan_path = os.path.join(OUTPUT, "state", f"monthly_plan_T{m:02d}.json")
...
def calc_metrics(all_rows, plan_lookup, route_order):
    ...
    metrics = {
        ...
        "plan_compliance": defaultdict(lambda: defaultdict(lambda: {"correct": 0, "wrong": 0})),
    ...
    for (date, tuyen, kho), actual_stops in trip_dests.items():
        ...
        # Route compliance (all-or-nothing per route)
        ...
        # Plan compliance (per-destination rank comparison)
        # Compare actual rank vs planned rank for each store
        planned_set = set(planned_order)
        # Dedup actual: keep first arrival at each store
        seen_stores = set()
        actual_deduped = []
        for _, s in actual_stops:
            if s not in seen_stores:
                seen_stores.add(s)
                actual_deduped.append(s)
        
        # Only compare stores in both actual and planned
        common_actual = [s for s in actual_deduped if s in planned_set]
        common_planned = [s for s in planned_order if s in seen_stores]
        
        if len(common_actual) >= 1 and len(common_actual) == len(common_planned):
            # Build rank maps
            actual_rank_map = {s: i for i, s in enumerate(common_actual)}
            planned_rank_map = {s: i for i, s in enumerate(common_planned)}
            
            kho_keys = [kho]
            if kho == "ĐÔNG MÁT":
                pass  # Will handle sub_kho below per store
            
            for store in common_actual:
                a_rank = actual_rank_map[store]
                p_rank = planned_rank_map[store]
                is_match = (a_rank == p_rank)
                
                # Record for main kho
                if is_match:
                    metrics["plan_compliance"][kho][date]["correct"] += 1
                else:
                    metrics["plan_compliance"][kho][date]["wrong"] += 1
                
                # Record for sub_kho (ĐÔNG / MÁT)
                if kho == "ĐÔNG MÁT":
                    sk = dest_sub_kho.get((date, store), "")
                    if sk:
                        if is_match:
                            metrics["plan_compliance"][sk][date]["correct"] += 1
                        else:
                            metrics["plan_compliance"][sk][date]["wrong"] += 1
...
def prepare_chart_data(metrics, dates, month, year):
    ...
    # ── Chart 4: Plan compliance (per-destination rank match) ──
    pc_corrects = []
    pc_wrongs = []
    for d in dates:
        c = sum(metrics["plan_compliance"].get(k, {}).get(d, {}).get("correct", 0) for k in khos)
        w = sum(metrics["plan_compliance"].get(k, {}).get(d, {}).get("wrong", 0) for k in khos)
        pc_corrects.append(c)
        pc_wrongs.append(w)
    
    kho_pc_bars = {}
    for kho in khos:
        if kho not in metrics["plan_compliance"]:
            continue
        k_c = []
        k_w = []
        for d in dates:
            data = metrics["plan_compliance"].get(kho, {}).get(d, {})
            k_c.append(data.get("correct", 0))
            k_w.append(data.get("wrong", 0))
        kho_pc_bars[kho] = {"on": k_c, "late": k_w}
    ...
    for kho in khos:
        ...
        for d in dates:
            ...
            total = c + w
            pcts.append(round(c / total * 100, 1) if total > 0 else None)
        pc_trends.append({"label": kho, "data": pcts, "color": KHO_COLORS[kho]})
    
    charts["plan_compliance"] = {
        "corrects": pc_corrects, "wrongs": pc_wrongs,
        "kho_bars": kho_pc_bars,
        "trends": pc_trends,
        "avg": avg_pct("plan_compliance", "correct", "wrong"),
    }
    
    # ── Chart 5: Trips + Completion (stacked bar + trend) ──
    ...
    return labels, charts, iso_dates
...
def generate_weekly_tables(metrics, dates):
    ...
    def compute_plan_compliance_row(kho, col_keys):
        """Compute % Đúng Kế Hoạch row for weekly tables."""
        pct_vals = []
        for wk_key, day_date in col_keys:
            if day_date is None:
                wk_dates = weeks[wk_key]["dates"]
                total_c = sum(metrics["plan_compliance"].get(kho, {}).get(d, {}).get("correct", 0) for d in wk_dates)
                total_w = sum(metrics["plan_compliance"].get(kho, {}).get(d, {}).get("wrong", 0) for d in wk_dates)
            else:
                pc = metrics["plan_compliance"].get(kho, {}).get(day_date, {})
                total_c = pc.get("correct", 0)
                total_w = pc.get("wrong", 0)
            total = total_c + total_w
            pct = round(total_c / total * 100, 1) if total > 0 else ""
            pct_vals.append(f"{pct}%" if pct != "" else "")
        return pct_vals
    ...
    for kho, title, use_sla, add_sla_row in table_configs:
        ...
        if add_sla_row:
            ...
            for k, v in ordered_data.items():
                if k == "Tổng Số Chuyến":
                    final_data["% Đúng Kế Hoạch"] = pc_pct_vals
                final_data[k] = v
            if "% Đúng Kế Hoạch" not in final_data:
                final_data["% Đúng Kế Hoạch"] = pc_pct_vals
            ordered_data = final_data
...
def generate_summary_cards(metrics, all_rows):
    ...
    for kho in KHO_COLORS:
        pc_data = metrics["plan_compliance"].get(kho, {})
        total_c = sum(d.get("correct", 0) for d in pc_data.values())
        total_w = sum(d.get("wrong", 0) for d in pc_data.values())
        total = total_c + total_w
        cards[f"plancmp_{kho}"] = {"total": total, "pct": total_c / total * 100 if total > 0 else 0}
    
    return cards


def build_html_report(labels_json, charts_json, cards, month, year, weekly_tables_html="", iso_dates_json="[]"):
    ...
    def card_html(title, key_prefix):
        ...
        for kho in KHO_COLORS:
            ...
            safe_kho = kho.replace(' ', '_').replace('-', '_')
            rows_html += f"""
            <tr data-card="{key_prefix}_{safe_kho}">
                <td class="td-kho">{kho}</td>
                <td class="td-num" id="c-{key_prefix}-{safe_kho}-t">{total}</td>
                <td class="td-pct" id="c-{key_prefix}-{safe_kho}-p" style="color:{color}">{pct:.1f}%</td>
            </tr>"""
        return f"""
        <div class="card">
            <h3>{title}</h3>
...
<head>
    ...
    <style>
        * {{ margin:0; padding:0; box-sizing:border-box; }}
        ...
        .date-filter label {{ color:#A8BCFF; font-size:13px; font-weight:600; }}
        .date-filter input[type=date] {{
            background:rgba(255,255,255,0.08); border:1px solid rgba(255,255,255,0.15);
            border-radius:8px; color:#E0E0E0; padding:6px 12px; font-size:13px;
            font-family:inherit; cursor:pointer;
        }}
        .date-filter input[type=date]::-webkit-calendar-picker-indicator {{ filter:invert(0.8); }}
        .date-filter .reset-btn {{
            padding:6px 16px; border-radius:8px; border:1px solid rgba(255,255,255,0.15);
            background:rgba(231,76,60,0.15); color:#E74C3C; cursor:pointer; font-size:12px;
            font-family:inherit; transition:all 0.2s;
        }}
        .date-filter .reset-btn:hover {{ background:rgba(231,76,60,0.3); }}
        .date-filter .range-info {{ color:#888; font-size:12px; margin-left:auto; }}
    </style>
</head>
<body>
<div class="container">
    ...
    <div class="cards">
        ...
        {card_html("📅 On Time Per Destination", "plan")}
        {card_html("🗺 Đúng Tuyến", "route")}
        {card_html("📋 Đúng Kế Hoạch", "plancmp")}
    </div>

    <div class="chart-box" style="margin-bottom:24px">
        <h3>📖 Chú Thích Các Chỉ Tiêu</h3>
        <table style="width:100%;border-collapse:collapse;font-size:13px;margin-top:12px">
            <thead><tr style="background:rgba(255,255,255,0.05)"><th style="padding:8px 12px;text-align:left;color:#A8BCFF">Chỉ Tiêu</th><th style="padding:8px 12px;text-align:left;color:#A8BCFF">Ý Nghĩa</th></tr></thead>
            <tbody>
                <tr><td style="padding:8px 12px;border-bottom:1px solid #333;color:#ccc;font-weight:600">% On Time SLA</td><td style="padding:8px 12px;border-bottom:1px solid #333;color:#aaa">Thời gian đến siêu thị thực tế nằm trong khung giờ cam kết (SLA) của từng kho</td></tr>
                <tr><td style="padding:8px 12px;border-bottom:1px solid #333;color:#ccc;font-weight:600">% On Time Per Destination</td><td style="padding:8px 12px;border-bottom:1px solid #333;color:#aaa">Thời gian giao đến siêu thị thực tế ≤ thời gian dự kiến giao theo kế hoạch</td></tr>
                <tr><td style="padding:8px 12px;border-bottom:1px solid #333;color:#ccc;font-weight:600">% Đúng Tuyến</td><td style="padding:8px 12px;border-bottom:1px solid #333;color:#aaa">Thứ tự giao hàng toàn tuyến khớp với kế hoạch</td></tr>
                <tr><td style="padding:8px 12px;border-bottom:1px solid #333;color:#ccc;font-weight:600">% Đúng Kế Hoạch</td><td style="padding:8px 12px;border-bottom:1px solid #333;color:#aaa">Tỉ lệ điểm giao được thực hiện đúng thứ tự tuyến kế hoạch (so vị trí từng điểm)</td></tr>
                <tr><td style="padding:8px 12px;border-bottom:1px solid #333;color:#ccc;font-weight:600">% Hoàn Thành</td><td style="padding:8px 12px;border-bottom:1px solid #333;color:#aaa">Tỉ lệ chuyến xe hoàn thành giao hàng / tổng số chuyến</td></tr>
            </tbody>
        </table>
    </div>
    ...
    <div class="chart-box"><h3>2. On Time Per Destination</h3><span class="filter-bar" id="filterPlan"></span><canvas id="chartPlan"></canvas></div>
    <div class="chart-box"><h3>3. Tỷ Lệ Đúng Tuyến</h3><span class="filter-bar" id="filterRoute"></span><canvas id="chartRoute"></canvas></div>
    <div class="chart-box"><h3>4. % Đúng Kế Hoạch</h3><span class="filter-bar" id="filterPlanCmp"></span><canvas id="chartPlanCmp"></canvas></div>
    <div class="chart-box"><h3>5. Số Chuyến & Tỷ Lệ Hoàn Thành</h3><span class="filter-bar" id="filterTripsComp"></span><canvas id="chartTripsComp"></canvas></div>
...
function initPlan(kho) {{ buildBarTrend('chartPlan', CHARTS.plan, 'Đúng KH', 'Trễ KH', '% On Time Per Dest', 95, kho); }}
function initRoute(kho) {{ buildBarTrend('chartRoute', CHARTS.route, 'Đúng Tuyến', 'Sai Tuyến', '% Đúng Tuyến', null, kho, 'Số điểm đến'); }}
function initPlanCmp(kho) {{ buildBarTrend('chartPlanCmp', CHARTS.plan_compliance, 'Đúng KH', 'Sai KH', '% Đúng Kế Hoạch', null, kho, 'Số điểm đến'); }}
...
makeFilterBtns('filterPlanCmp', ['chartPlanCmp'], initPlanCmp);
makeFilterBtns('filterTripsComp', ['chartTripsComp'], initTripsComp);

// ── Date range filter ──
const ISO_DATES = {iso_dates_json};
const FULL_LABELS = [...LABELS];
const FULL_CHARTS = JSON.parse(JSON.stringify(CHARTS));

// Card mapping: prefix -> chart key
const CARD_MAP = {{
    sla: 'sla', plan: 'plan', route: 'route', plancmp: 'plan_compliance'
}};
// Safe kho names for IDs
const SAFE_KHO = {{}};
KHO_NAMES.forEach(k => SAFE_KHO[k] = k.replace(/ /g, '_').replace(/-/g, '_'));

(function initDatePicker() {{
    if (ISO_DATES.length === 0) return;
    const df = document.getElementById('dateFrom');
    const dt = document.getElementById('dateTo');
    df.min = ISO_DATES[0]; df.max = ISO_DATES[ISO_DATES.length - 1];
    dt.min = ISO_DATES[0]; dt.max = ISO_DATES[ISO_DATES.length - 1];
    df.value = ISO_DATES[0];
    dt.value = ISO_DATES[ISO_DATES.length - 1];
    updateRangeInfo();
}})();
...
function updateRangeInfo() {{
    ...
    if (!df || !dt) return;
    const startIdx = ISO_DATES.findIndex(d => d >= df);
    let endIdx = ISO_DATES.length;
    for (let i = 0; i < ISO_DATES.length; i++) {{ if (ISO_DATES[i] > dt) {{ endIdx = i; break; }} }}
    const cnt = endIdx - Math.max(startIdx, 0);
    document.getElementById('rangeInfo').textContent = `${{cnt}} ngày`;
}}

function sliceArr(arr, s, e) {{ return arr ? arr.slice(s, e) : arr; }}
...
function applyDateRange() {{
    ...
    let startIdx = ISO_DATES.findIndex(d => d >= df);
    ...
    // Slice labels
    LABELS.length = 0;
    FULL_LABELS.slice(startIdx, endIdx).forEach(l => LABELS.push(l));
    
    // Slice chart data
    for (const ck of Object.keys(FULL_CHARTS)) {{
        const src = FULL_CHARTS[ck];
        const dst = CHARTS[ck];
        dst.on_times = sliceArr(src.on_times, startIdx, endIdx);
        dst.lates = sliceArr(src.lates, startIdx, endIdx);
        dst.corrects = sliceArr(src.corrects, startIdx, endIdx);
        dst.wrongs = sliceArr(src.wrongs, startIdx, endIdx);
        dst.avg = sliceArr(src.avg, startIdx, endIdx);
        if (src.kho_bars) {{
            for (const k of Object.keys(src.kho_bars)) {{
                if (!dst.kho_bars[k]) dst.kho_bars[k] = {{}};
                dst.kho_bars[k].on = src.kho_bars[k].on.slice(startIdx, endIdx);
                dst.kho_bars[k].late = src.kho_bars[k].late.slice(startIdx, endIdx);
            }}
        }}
        if (src.trends) {{
            for (let i = 0; i < src.trends.length; i++) {{
                dst.trends[i].data = src.trends[i].data.slice(startIdx, endIdx);
            }}
        }}
    }}
    
    // Re-init all charts with current filter state
    ['filterSLA','filterPlan','filterRoute','filterPlanCmp','filterTripsComp'].forEach(id => {{
        const active = document.getElementById(id)?.querySelector('.fbtn.active');
        const kho = active && active.textContent !== 'Tổng' ? active.textContent : null;
        if (id === 'filterSLA') initSLA(kho);
        else if (id === 'filterPlan') initPlan(kho);
        else if (id === 'filterRoute') initRoute(kho);
        else if (id === 'filterPlanCmp') initPlanCmp(kho);
        else if (id === 'filterTripsComp') initTripsComp(kho);
    }});
    
    // Update cards
    for (const [prefix, chartKey] of Object.entries(CARD_MAP)) {{
        const bars = CHARTS[chartKey]?.kho_bars;
        if (!bars) continue;
        for (const kho of KHO_NAMES) {{
            const sk = SAFE_KHO[kho];
            const b = bars[kho];
            if (!b) continue;
            const on = b.on.reduce((a, v) => a + v, 0);
            const late = b.late.reduce((a, v) => a + v, 0);
            const total = on + late;
            const pct = total > 0 ? (on / total * 100) : 0;
            const el_t = document.getElementById(`c-${{prefix}}-${{sk}}-t`);
            const el_p = document.getElementById(`c-${{prefix}}-${{sk}}-p`);
            if (el_t) el_t.textContent = total;
            if (el_p) {{
                el_p.textContent = pct.toFixed(1) + '%';
                el_p.style.color = pct >= 95 ? '#2ECC71' : pct >= 90 ? '#F39C12' : '#E74C3C';
            }}
        }}
    }}
...
function resetDateRange() {{
    if (ISO_DATES.length === 0) return;
    document.getElementById('dateFrom').value = ISO_DATES[0];
    document.getElementById('dateTo').value = ISO_DATES[ISO_DATES.length - 1];
    applyDateRange();
}}

initSLA(null); initPlan(null); initRoute(null); initPlanCmp(null); initTripsComp(null);
</script>
</body>
</html>"""
    return html
...
def main():
    ...
    # Parse end-date cutoff
    end_date = None
    if args.end_date:
        try:
            parts = args.end_date.split('/')
            end_date = ddate(int(parts[2]), int(parts[1]), int(parts[0]))
            print(f"  📅 End date cutoff: {end_date.strftime('%d/%m/%Y')}")
        except Exception as e:
            print(f"  ⚠ Invalid --end-date format: {args.end_date} (use DD/MM/YYYY)")
            return
    ...
    if end_date:
        before_trip = len(trip_rows)
        trip_rows = [r for r in trip_rows if r.get("date") and r["date"] <= end_date]
        before_tc = len(thitca_rows)
        thitca_rows = [r for r in thitca_rows if r.get("date") and r["date"] <= end_date]
        print(f"  🔒 End-date filter: trips {before_trip}→{len(trip_rows)}, THỊT CÁ {before_tc}→{len(thitca_rows)}")
    ...
    # Print summary
    for metric_name in ["sla", "plan", "route", "plan_compliance"]:
        print(f"\n  {metric_name.upper()}:")
        for kho in KHO_COLORS:
    ...
    # Apply end-date cutoff to possible dates
    if end_date:
        all_possible_dates = [d for d in all_possible_dates if d <= end_date]
    ...
    labels, chart_data, iso_dates = prepare_chart_data(metrics, dates, months[0], year)
    labels_json = json.dumps(labels, ensure_ascii=False)
    charts_json = json.dumps(chart_data, ensure_ascii=False)
    iso_dates_json = json.dumps(iso_dates, ensure_ascii=False)
    
    # 4. Generate HTML
    print("\n📝 Generating HTML report...")
    cards = generate_summary_cards(metrics, all_rows)
    weekly_html = generate_weekly_tables(metrics, dates)
    html = build_html_report(labels_json, charts_json, cards, months[0], year, weekly_html, iso_dates_json)
    
    perf_dir = os.path.join(OUTPUT, "artifacts", "performance")
    os.makedirs(perf_dir, exist_ok=True)
    out_path = os.path.join(perf_dir, f"PERFORMANCE_REPORT_{month_str}_{year}.html")
